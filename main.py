...
GENDER, PHOTO, STARSIGN, LOCATION, BIO = range(5)
...
def photo(update, _):
    ...
    # Список кнопок для ответа
    star_sign_keyboard = [['Овен', 'Телец', 'Близнецы', 'Рак', 'Лев', 'Дева', 'Весы', 'Скорпион', 'Стрелец', 'Козерог', 'Водолей', 'Рыбы']]
    # Создаем простую клавиатуру для ответа
    markup_key = ReplyKeyboardMarkup(star_sign_keyboard, one_time_keyboard=True)

    # Отвечаем на сообщение с фото
    update.message.reply_text(
        'Великолепно! А теперь выбери свой знак зодиака: '
        'Команда /skip, если не веришь во всё это.',
        reply_markup=markup_key,
    )

    # переходим к этапу `STARSIGN`
    return STARSIGN
...
def skip_photo(update, _):
    ...
    # Список кнопок для ответа
    star_sign_keyboard = [['Овен', 'Телец', 'Близнецы', 'Рак', 'Лев', 'Дева', 'Весы', 'Скорпион', 'Стрелец', 'Козерог', 'Водолей',
         'Рыбы']]
    # Создаем простую клавиатуру для ответа
    markup_key = ReplyKeyboardMarkup(star_sign_keyboard, one_time_keyboard=True)
    ...
    update.message.reply_text(
        'Держу пари, ты выглядишь великолепно! А теперь выбери свой знак зодиака: '
        'Команда /skip, если не веришь во всё это.',
        reply_markup=markup_key,
    )
    # переходим к этапу `STARSIGN`
...
def star_sign(update, _):
    ...
    # Начинаем разговор с вопроса
    update.message.reply_text(
        'Великолепно! А теперь пришлите мне'
...
def skip_star_sign(update, _):
    # определяем пользователя
    user = update.message.from_user
    update.message.reply_text(
        'Я тоже не верю в знаки зодиака и гороскопы! А теперь пришлите мне'
        ' свое местоположение, или /skip если параноик.,',
        reply_markup=ReplyKeyboardRemove(),
    )
    # переходим к этапу `LOCATION`
...
if __name__ == '__main__':
    ...
    updater = Updater("6011938072:AAF2T5rTiP_sNRLlrXYuoM8Wf-2XBKP7Qgo")
    # получаем диспетчера для регистрации обработчиков
    dispatcher = updater.dispatcher

    # Определяем обработчик разговоров `ConversationHandler`
    # с состояниями GENDER, PHOTO, STARSIGN, LOCATION и BIO
    conv_handler = ConversationHandler( # здесь строится логика разговора
        ...
        states={
            ...
            STARSIGN: [
                MessageHandler(Filters.regex('^(Овен|Телец|Близнецы|Рак|Лев|Дева|Весы|Скорпион|Стрелец|Козерог|Водолей|Рыбы)$'),star_sign),
                CommandHandler('skip', skip_star_sign),
            ],
            LOCATION: [
